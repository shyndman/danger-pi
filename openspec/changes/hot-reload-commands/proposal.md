...
1. Keep scope native and fork-local.
   - Auto-reload covers native OMP roots only: `<cwd>/.omp` and `~/.omp/agent`.
...
3. Implement watcher behavior using additive hooks, not structural rewrites.
   ...
   - Integrate helper through small call sites in interactive mode lifecycle (init, setting change handling, shutdown, and `/move` path transitions).
...
6. Add persistent watcher error visibility.
   ...
   - This differs intentionally from existing silent watcher patterns.
...
- Non-functional constraints:
  ...
  - No new external dependency is required for this version; existing code already uses `fs.watch` patterns.
