...
export class OmpLiveReloadController {
	...
	/**
	 * Updates live-reload mode/roots and rebinds watchers immediately.
	 *
	 * Key API goal: make init, /move, and settings changes share one safe rebind path.
	 */
	...
	/**
	 * Force a watcher rebind and optional refresh pass.
	 *
	 * Key API goal: provide deterministic retry behavior for manual /reload recovery.
	 */
	...
	#formatWatcherError(error: unknown): string {
		...
		if (code === "ENOSPC") {
			return `OMP live reload failed (${code}): inotify watch limit reached. Increase fs.inotify.max_user_watches, then run /reload.`;
		}
