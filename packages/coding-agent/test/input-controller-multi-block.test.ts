...
class StubEditor {
	...
	public pendingImageLinks: (string | undefined)[] = [];
	public imageLinks?: readonly (string | undefined)[];
...
describe("InputController multi-block submissions", () => {
	...
	it("flushes earlier bash shortcut output before starting a final prompt-chain command", async () => {
		...
		const onInput = vi.fn();
		ctx.onInputCallback = onInput;
		ctx.editor.pendingImages = [{ type: "image", data: "diagram", mimeType: "image/png" }];
