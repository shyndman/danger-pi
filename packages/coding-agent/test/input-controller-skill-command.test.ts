...
class StubEditor {
	...
	clearDraft(historyText?: string): void {
		if (historyText) this.history.push(historyText);
		this.text = "";
...
function createTestContext() {
	...
	const startPendingSubmission = vi.fn(
		(input: { text: string; images?: InteractiveModeContext["editor"]["pendingImages"] }): SubmittedUserInput => ({
			text: input.text,
			images: input.images,
			cancelled: false,
			started: false,
...
describe("InputController skill commands", () => {
	it("includes the anti-reread note in the injected skill prompt", async () => {
		...
		ctx.skillCommands.set("skill:demo-skill", {
			filePath: skillPath,
			isNative: false,
			name: "demo-skill",
		...
		try {
			...
			expect(options).toEqual(expect.objectContaining({ streamingBehavior: "steer" }));
		} finally {
	...
	it("interpolates native skill bodies before appending metadata lines", async () => {
		...
		ctx.skillCommands.set("skill:native-skill", {
			filePath: skillPath,
			isNative: true,
			name: "native-skill",
			baseDir: path.dirname(skillPath),
	...
	it("keeps native skill frontmatter literal by expanding only the parsed body", async () => {
		...
		await Bun.write(skillPath, '---\ndescription: "!`false`"\n---\nLiteral body');
		ctx.sessionManager.getCwd = () => tempDir;
		ctx.skillCommands.set("skill:frontmatter-skill", {
			filePath: skillPath,
			isNative: true,
	...
	it("leaves non-native skill bodies literal even when they contain shell syntax", async () => {
		...
		await Bun.write(skillPath, "---\ndescription: Demo skill\n---\nLiteral !`false`");
		ctx.skillCommands.set("skill:imported-skill", {
			filePath: skillPath,
			isNative: false,
			name: "imported-skill",
			baseDir: path.dirname(skillPath),
